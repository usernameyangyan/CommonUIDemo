 ...
 *Describe:
 */
class YYDialog : BaseDialog(),
    IDialog {
    private var context: Context? = null
    private var controller: DialogController? = null
    ...
    class Builder(context: Context) {
        private val params: DialogController.YYParams
        private var buildListener: IDialog.OnBuildListener? = null
        private var dismissListener: IDialog.OnDismissListener? = null
        ...
        private fun create(): YYDialog {
            val YYDialog =
                YYDialog()
            params.apply(YYDialog.controller!!)
            YYDialog.buildListener = buildListener
            YYDialog.dismissListener = dismissListener
            return YYDialog
        }
        ...
        fun show(): YYDialog {
            ...
            val YYDialog: YYDialog = create()
            //判空
            if (params.context == null) return YYDialog
            if (params.context is Activity) {
                val activity = params.context as Activity
                //如果Activity正在关闭或者已经销毁 直接返回
                val isRefuse =
                    activity.isFinishing || activity.isDestroyed
                if (isRefuse) return YYDialog
            }
            removePreDialog()
            YYDialog.showAllowingLoss(
